...
export default (({
  ...
  class Provider extends Component {
    constructor(props) {
      ...
      this.actions = mapValues(actions(props, this.setStateAndSave.bind(this)), action => async (...args) => {
        const newState = await action(...args);
        if (newState) this.setStateAndSave(newState);
      });
    }
...
export const combine = (...items) => {
  ...
  return (...args) => items.reduce((acc, item) => assign({}, acc, tie(item)(...args)), {});
};